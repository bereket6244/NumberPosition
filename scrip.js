...
const instructionOpener = document.getElementsByClassName(
  "instructions-opener"
)[0];
const instructionsOverlay = document.getElementsByClassName(
  "instructions-overlay"
)[0];
const alertContainer = document.querySelector("[data-alert-container]");

// let bal = localStorage.getItem("blala"); awesome innovation
// console.log(bal);
let score = 0;
...
const saveToLocalStorage = () => {
  if (localStorage.getItem("score") === null) {
    ...
    //   `tried amount ${Date.prototype.toLocaleString("en-US")}`,
    //   triedAmount
    // );
  }
  if (localStorage.getItem("score") != null) {
...
function checkNumPresence() {
  ...
  if (
    ...
    first.innerHTML === third.innerHTML ||
    first.innerHTML === fourth.innerHTML ||
    second.innerHTML === third.innerHTML ||
    second.innerHTML === fourth.innerHTML ||
    ...
    if (
      ...
      for (let j = 0; j <= 3; j++) {
        for (let i = 0; i <= 3; i++) {
          if (
            document.getElementsByClassName("guess")[j].innerText === ranNum[i]
          ) {
            correctNumber++;
            if (j === i) {
              correctPosition++;
